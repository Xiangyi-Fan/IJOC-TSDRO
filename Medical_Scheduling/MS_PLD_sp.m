function [x,obj,runtime] =  MS_PLD_sp(cons_points,data,epsilon0_sp,K)
...
for k = 1:K
    ...
    if I_k > 0
        epsilon{k,1} = epsilon0_sp/(sqrt(I_k)*K);
    else
        epsilon{k,1} = epsilon0_sp;
    end
end
...
for k = 1:K; theta1{k} = sdpvar(p,1);end
for k = 1:K; theta2{k} = sdpvar(p,1);end
for k = 1:K; theta3{k} = sdpvar(p,1);end
