...
%     epsilon_wass_list = zeros(trial_new,1);
i = 0;
while i < trial_new
    i = i+1;
    epsilon_cop_list(i) = LS_epsilon_cop_value(data_cons0,data0_in{i,1});
    epsilon_sp_list(i) = LS_epsilon_sp_value(data_cons0,data0_in{i,1});
end
...
while i < n
    ...
    N = Npoints(i)
    K = N;
    [avg_cop,avg_sp,avg_saa,avg_wass,cop0,cop100,sp0,sp100,saa0,saa100,wass0,wass100,feasibility_num,avg_T_cop,avg_T_sp,avg_T_saa,avg_T_wass] = outperformance_LS(N,epsilon_cop_fix,epsilon_sp_fix);
...
plot_wass = plot_with_shade(Npoints(1:num), avg_wass_list(1:num), avg_wass_list_neg0(1:num),avg_wass_list_pos100(1:num), alpha, color(3,:),'o');
...
lgd = legend([plot_cop plot_sp plot_wass plot_saa], 'C1','C0','Wasserstein','SAA','Location', 'northeast');
