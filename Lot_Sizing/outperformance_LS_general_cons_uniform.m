function [avg_cop,avg_sp,avg_saa,avg_wass,cop0,cop100,sp0,sp100,saa0,saa100,wass0,wass100,feasibility_num,avg_T_cop,avg_T_sp,avg_T_saa,avg_T_wass] = outperformance_LS_general_cons_uniform(Npoints,epsilon_cop_fix,epsilon_sp_fix,epsilon_cop_p_fix,epsilon_sp_p_fix)
...
data_cons = LS_generate_data_uniform(Npoints);
...
for i = 1:trial_new
    ...
    % coposiitive 
    if cop_cont
        [x_cop,obj_cop,runtime_cop] = LS_PLD_cop_general(data_cons,data_in{i,1},epsilon_cop,epsilon_cop_p_fix);   
    ...
    % Slemma 
    if sp_cont
        [x_sp,obj_sp,runtime_sp] = LS_PLD_sp_general(data_cons,data_in{i,1},epsilon_sp,epsilon_sp_p_fix);   
        if runtime_sp > 30*60
            sp_cont = false;
            T_dro_sp(i) = NaN;
            dro_sp_list(i) = NaN;
