function [avg_cop,avg_sp,avg_saa,avg_wass,cop0,cop100,sp0,sp100,saa0,saa100,wass0,wass100,feasibility_num,avg_T_cop,avg_T_sp,avg_T_saa,avg_T_wass] = outperformance_LS(Npoints,epsilon_cop_fix,epsilon_sp_fix)
...
data_cons0 = LS_generate_data(20);
...
for i = 1:trial_new
    ...
    % coposiitive 
    if cop_cont
        [x_cop,obj_cop,runtime_cop] = LS_PLD_cop(data_cons,data_in{i,1},epsilon_cop);   
    ...
    % Slemma 
    if sp_cont
        [x_sp,obj_sp,runtime_sp] = LS_PLD_sp(data_cons,data_in{i,1},epsilon_sp);   
        if runtime_sp > 30*60
            sp_cont = false;
            T_dro_sp(i) = NaN;
            dro_sp_list(i) = NaN;
