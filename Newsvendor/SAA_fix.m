...
W0 = [[1,zeros(1,N),zeros(1,N)];[1,-g',-s'];[zeros(N,1),eye(N),zeros(N)];[zeros(N,1),zeros(N),eye(N)];[zeros(N,1),eye(N),zeros(N)];[zeros(N,1),zeros(N),eye(N)]];
M0 = size(T0,1);
d0 = 2*N;
N_2 = size(r0,1);
obj0 = 0;
